...
class TestPriceCache:
    ...
    def test_remove_bumps_version(self):
        """Removing a present ticker bumps version so SSE clients notice."""
        cache = PriceCache()
        cache.update("AAPL", 190.00)
        version_after_update = cache.version
        cache.remove("AAPL")
        assert cache.version == version_after_update + 1
    ...
    def test_remove_nonexistent_does_not_bump_version(self):
        """No-op remove should not change the version counter."""
        cache = PriceCache()
        cache.update("AAPL", 190.00)
